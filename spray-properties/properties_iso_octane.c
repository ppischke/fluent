...
real property_iso_octane_vapor_diffusivity(real T, real p)
{


	real vapor_diffusivity;

	return vapor_diffusivity = 1.013e-2*pow(T , 1.75)*pow((M_ISO_OCTANE+29.0)/(29.0*M_ISO_OCTANE) , 0.5)/(p*pow2(pow(19.7,1/3)+pow(168.78 , 1/3)));
}
...
DEFINE_DPM_PROPERTY(iso_octane_liq_viscosity, c, t, p)
{
	return property_iso_octane_liq_viscosity(P_T(p));
}



DEFINE_DPM_PROPERTY(iso_octane_liq_surface_tension, c, t, p)
{	
	return property_iso_octane_liq_surface_tension(P_T(p));
}



DEFINE_DPM_PROPERTY(iso_octane_liq_specific_heat, c, t, p)
{
	return property_iso_octane_liq_specific_heat(P_T(p));
}



DEFINE_DPM_PROPERTY(iso_octane_liq_density, c, t, p)
{
	return property_iso_octane_liq_density(P_T(p));
}



DEFINE_DPM_PROPERTY(iso_octane_heat_vaporization, c, t, p)
{
	return property_iso_octane_heat_vaporization(P_T(p));
}



DEFINE_DPM_PROPERTY(iso_octane_vapor_pressure, c, t, p)
{	
	return property_iso_octane_vapor_pressure(P_T(p));
}



DEFINE_PROPERTY(iso_octane_vapor_diffusivity, c, t)
{
	return property_iso_octane_vapor_diffusivity(C_T(c,t));
}
